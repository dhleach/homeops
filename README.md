...
- **1348 Python tests + 51 React component tests**, GitHub Actions CI, Ruff lint/format enforcement on every PR, and post-deploy public smoke checks
...
- **Fleet Deploy Lab** — a bounded, simulated-fleet deployment control-plane demo is being built separately from normal HomeOps releases; its shared `DeploymentSpec` contract, durable SQLite state boundary, public-read/protected-management API, and responsive read-only `/deploy` fleet view live in `deploy_demo/`, `dashboard/backend/fleet_api.py`, and `dashboard/frontend/src/components/FleetDeployView.jsx`, with design notes in [`deploy-demo/`](deploy-demo/)
