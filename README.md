...
- **1348 Python tests + 53 React component tests**, GitHub Actions CI, Ruff lint/format enforcement on every PR, and post-deploy public smoke checks
...
- **Fleet Deploy Lab** — a bounded, simulated-fleet deployment control-plane demo is being built separately from normal HomeOps releases; its shared `DeploymentSpec` contract, durable SQLite state boundary, public-read/protected-management API, responsive read-only `/deploy` fleet view, and preview-only constrained form live in `deploy_demo/`, `dashboard/backend/fleet_api.py`, and `dashboard/frontend/src/components/`, with design notes in [`deploy-demo/`](deploy-demo/)
...
├── deploy_demo/                   # Shared Fleet Deploy Lab contracts and state
