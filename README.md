...
- **38 derived event types** from raw HA state changes and explicit HA mitigation decisions — heating/cooling floor-call sessions, furnace and inferred-AC sessions, per-zone heating/cooling thermal outcomes, furnace diagnostics, thermostat setpoint/mode/temp changes, zone temperature snapshots, outdoor temperature, daily summaries, system watchdog, staged zone-stagger outcomes, automatic mitigation rollbacks, and opt-in proactive anomaly explanations
- **Heating and cooling cycle analytics** — `zone_time_to_temp` / `zone_time_to_cool` (how fast each zone reaches its directional target), `zone_overshoot` / `zone_cooling_undershoot` (post-target behavior), `zone_setpoint_miss` / `zone_cooling_setpoint_miss`, and furnace duty cycle via `furnace_duty_cycle.py`
- **Thermostat entity tracking** — shared setpoint/mode/current-temp changes plus directional heat/cooling session, target, miss, and post-target events per zone
- **Mode-aware dashboard** — `/api/current-temps` preserves the legacy heating fields and adds null-safe cooling-call, inferred-AC, and per-zone heating/cooling/idle action fields; homeops.now renders those actions without inferring heating from legacy booleans
- **Cooling-aware Ask HomeOps** — the authenticated read-only diagnostic uses the same inferred cooling demand and per-zone action data, reports mixed or unavailable states honestly, and never claims compressor telemetry or thermostat control
...
- **Controlled thermal experiments** — a data-only natural-language marker sidecar records deliberate cooling tests, preserves repeat-run provenance, and lets offline export/evaluation keep each experiment together without controlling thermostats
...
- **1404 Python tests + 55 React component tests**, GitHub Actions CI, Ruff lint/format enforcement on every PR, and post-deploy public smoke checks
...
- **Fleet Deploy Lab** — a bounded, simulated-fleet deployment control-plane demo is being built separately from normal HomeOps releases; its shared `DeploymentSpec` contract, trusted master-only artifact validation and Actions deployment workflow, server-side manifest/dispatch admission, immutable-artifact Python deployer, durable SQLite state boundary, public-read/protected-management API, responsive `/deploy` fleet view, constrained submit form, and end-to-end simulator/deployer proof suite live in `deploy_demo/`, `.github/workflows/fleet-deploy.yml`, `dashboard/backend/fleet_api.py`, and `dashboard/frontend/src/components/`, with design notes in [`deploy-demo/`](deploy-demo/)
...
│        events.jsonl      (derived + insight audit)       │
│       (derived events)                                   │
└──────────────────────────────────────────────────────────┘
...
For the deterministic, no-live-writes mitigation replay and its remaining
Home Assistant validation boundary, see
...
For the canonical cooling-coupling experiment matrix, plain-language marker
workflow, and repeat-run policy, see
...
[`docs/bob-evaluation-publication.md`](docs/bob-evaluation-publication.md).
...
**Consumer** tails the observer log in real time. It routes each event by entity ID, maintains independent per-zone heating/cooling and whole-home furnace/AC session state, emits higher-level derived events, and exports live Prometheus metrics via `/metrics`. When explicitly enabled, validated runtime anomalies also pass through a bounded LLM explanation and Telegram delivery path.

**EC2 dashboard stack** — Prometheus scrapes the Pi's `/metrics` endpoint every 15 seconds over Tailscale and the backend's internal diagnostic metrics over EC2 loopback. Grafana reads from Prometheus and serves 4 provisioned dashboards with heating/cooling state, runtime, session history, outdoor correlation, and anomaly views. Cooling is explicitly labeled as inferred thermostat demand, not compressor telemetry. FastAPI queries Prometheus and exposes backward-compatible structured JSON at `/api/current-temps`, including nullable cooling demand and per-zone action state; Ask HomeOps requires a Cognito-verified bearer principal, independent per-user/IP quota state in loopback-only Valkey, a process-wide provider-call backstop, low-cardinality abuse/cost metrics, and a deterministic read-only prompt-safety boundary. The browser uses authorization code + PKCE and never receives a client secret. Nginx proxies the public API surfaces behind a TLS subdomain but does not expose backend `/metrics`. The active production stack uses Docker Compose with host networking; the Kubernetes manifests are a separate migration surface.
...
The consumer emits **38 derived event types**:

| Category | Events |
|---|---|
| Floor heating calls | `floor_call_started.v1`, `floor_call_ended.v1` |
| Floor cooling calls | `cooling_call_started.v1`, `cooling_call_ended.v1` |
...
| AC sessions | `cooling_session_started.v1`, `cooling_session_ended.v1` |
...
| Cooling thermostat sessions | `thermostat_cooling_session_started.v1`, `thermostat_cooling_session_ended.v1` |
| Cooling performance | `thermostat_cooling_setpoint_reached.v1`, `zone_time_to_cool.v1`, `zone_cooling_setpoint_miss.v1`, `zone_cooling_undershoot.v1` |
...
| LLM explanations | `proactive_anomaly_insight.v1` (default-off) |
...
├── dashboard/
│   ├── backend/                  # FastAPI — telemetry, diagnostic, and Fleet Deploy APIs
│   └── frontend/                 # React + Tailwind — homeops.now public site
│       └── public/bob/evals/     # Reviewed static Bob evaluation snapshot
├── ansible/                       # Fleet Deploy Lab Ansible playbook and logical-target inventory
├── deploy_demo/                   # Fleet Deploy Lab contracts, state, deployers, and trusted artifact builder
├── deploy-demo/                   # Fleet Deploy Lab integration and contract docs
├── compose/
...
│       ├── metrics.py             # Prometheus gauges and metrics endpoint
│       ├── hvac_context.py        # bounded structured context for LLM input
│       ├── proactive_insight.py   # opt-in anomaly explanation and delivery boundary
│       ├── requirements.txt
...
| `GEMINI_API_KEY` | _(unset)_ | Provider credential required only when proactive insight is enabled |
| `HOMEOPS_PROACTIVE_INSIGHT_ENABLED` | `false` | Enable bounded LLM explanations for validated runtime anomalies |
| `HOMEOPS_PROACTIVE_INSIGHT_PROVIDER` | `gemini` | Provider adapter name; unsupported values fail closed |
| `HOMEOPS_PROACTIVE_INSIGHT_MODEL` | `gemini-2.5-flash` | Model passed to the selected provider |
| `HOMEOPS_PROACTIVE_INSIGHT_LOOKBACK_HOURS` | `48` | HVAC context lookback, bounded to 1–168 hours |
| `HOMEOPS_PROACTIVE_INSIGHT_MAX_CONTEXT_CHARS` | `8000` | Maximum context sent to the provider |
| `HOMEOPS_PROACTIVE_INSIGHT_MAX_OUTPUT_CHARS` | `1200` | Maximum provider response retained/delivered |
| `HOMEOPS_PROACTIVE_INSIGHT_TIMEOUT_S` | `10` | Provider and Telegram timeout, bounded to 1–10 seconds |
| `HOMEOPS_PROACTIVE_INSIGHT_DAILY_BUDGET` | `3` | Maximum provider calls per UTC day |
| `HOMEOPS_PROACTIVE_INSIGHT_STATE` | `state/consumer/proactive-insight-state.json` | Atomic state for successful-ID deduplication and daily budget |
...
```bash
pip install pytest pytest-asyncio respx PyYAML ansible-core
...
1404 Python tests cover observer reconnect logic, consumer event derivation, heating and cooling call/session events, floor-2 long-call warning and escalation, thermostat tracking, heating and cooling cycle analytics, consumer state persistence, Prometheus metrics gauge updates, cooling metrics and daily reporting, the FastAPI backend and its mode-aware cooling contract, Ask HomeOps authentication/quota/budget/observability/prompt-safety guards, Fleet Deploy Lab public-read, protected-management, manifest-admission, GitHub-dispatch, Actions run/job reconciliation, and recovery contracts, flattened production-container import-path validation, deployment smoke checks, provisioned Grafana dashboard contracts, insights engine rules, historical anomaly replay/reporting, multi-zone impact analysis, hourly zone-call frequency reporting, the floor-call data-model contract, daily furnace temperature/runtime scatter export, the self-contained HTML trend report, temperature-adjusted runtime anomaly analysis, zone cooling-curve heat-loss analysis, per-zone furnace-runtime-per-degree efficiency analysis, per-zone time-to-temperature modeling, natural-language thermal query composition, multi-zone scheduling query safety, shared rule configuration validation, enabled-rule gates, outdoor-temperature storm detection, direct consumer import-path validation, staged Home Assistant mitigation configuration, event logging, automatic rollback, mitigation end-to-end replay, proactive anomaly insight, missing-context, replay-guard, thermal prediction target contract, thermal prediction feature schema, thermal prediction evaluation protocol, normalized thermal training-row export, thermal training-row validation and quarantine, offline thermal baseline/Ridge training and evaluation, thermal experiment marker parsing and checklist coverage, Fleet Deploy Lab DeploymentSpec validation and canonicalization, durable simulated-fleet state/idempotency/persistence, local Python/Ansible input and playbook verification, local deployer artifact/client/verification contracts, simulator/deployer end-to-end API behavior and failure assertions, trusted manifest/profile artifact validation, workflow/readiness validation, downloaded artifact provenance verification, simulator deployment workflow gating, publication-boundary checks, CloudFront route checks, and test-count validation. The frontend has 55 React component tests covering mode-aware cards, summaries, stale-state handling, the existing dashboard widgets, the responsive simulated-fleet route, the constrained DeploymentSpec submit form, and persisted Fleet Deploy run-state reconstruction. The canonical counts live in [`docs/test-counts.json`](docs/test-counts.json) and are verified against CI runner output.
...
See [`docs/mitigation-test-results.md`](docs/mitigation-test-results.md) for
the scenario, output, and the remaining isolated-HA validation gate.
...
When heating and cooling histories occupy different time ranges, add
`--split-strategy mode_aware` to produce separate chronological partitions for
each mode. The report records per-mode boundaries and the mode-aware artifacts
fit Ridge separately per mode; the default global report remains the stable
...
### `scripts/scheduling_query.py`

Build a bounded, read-only floor-2 schedule recommendation from the existing
thermal models. Given a floor-2 target, current/outdoor temperatures, and an
ISO-8601 deadline, it estimates when the primary call would need to start and
calculates conservative floor-1/floor-3 setpoint ceilings for the call window.
It uses the configured `rules.floor_2_long_call` threshold as a safety boundary,
reserves five minutes by default, and fails closed for sparse, stale,
extrapolated, or unsafe data. Current temperatures may be supplied explicitly
or inferred from fresh thermostat snapshots at or before `as_of`.
...
The output is a planning result, not a thermostat command: it does not call
Home Assistant, modify consumer state, emit a production event, or deploy the
staged mitigation overlay. See
[`docs/multi-zone-scheduling-query.md`](docs/multi-zone-scheduling-query.md)
for the contract, safety math, and sparse-data behavior.
