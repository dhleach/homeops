...
- **1364 Python tests + 53 React component tests**, GitHub Actions CI, Ruff lint/format enforcement on every PR, and post-deploy public smoke checks
...
- **Fleet Deploy Lab** — a bounded, simulated-fleet deployment control-plane demo is being built separately from normal HomeOps releases; its shared `DeploymentSpec` contract, immutable-artifact Python deployer, durable SQLite state boundary, public-read/protected-management API, responsive read-only `/deploy` fleet view, preview-only constrained form, and end-to-end simulator/deployer proof suite live in `deploy_demo/`, `dashboard/backend/fleet_api.py`, and `dashboard/frontend/src/components/`, with design notes in [`deploy-demo/`](deploy-demo/)
...
├── deploy_demo/                   # Fleet Deploy Lab contracts, state, and deployer
