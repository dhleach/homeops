...
- **1347 Python tests + 48 React component tests**, GitHub Actions CI, Ruff lint/format enforcement on every PR, and post-deploy public smoke checks
...
- **Fleet Deploy Lab** — a bounded, simulated-fleet deployment control-plane demo is being built separately from normal HomeOps releases; its shared `DeploymentSpec` contract, durable SQLite state boundary, and public-read/protected-management API live in `deploy_demo/` and `dashboard/backend/fleet_api.py`, with design notes in [`deploy-demo/`](deploy-demo/)
