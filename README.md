...
- **1329 Python tests + 48 React component tests**, GitHub Actions CI, Ruff lint/format enforcement on every PR, and post-deploy public smoke checks
...
- **Fleet Deploy Lab contract** — a bounded, simulated-fleet deployment control-plane demo is being built separately from normal HomeOps releases; its strict shared `DeploymentSpec` contract lives in `deploy_demo/` and [`deploy-demo/deployment-contract.md`](deploy-demo/deployment-contract.md)
...
├── deploy_demo/                   # Shared Fleet Deploy Lab request contracts
